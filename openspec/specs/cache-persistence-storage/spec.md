# Cache Persistence Storage Specification

## Purpose

Defines the storage-tier contract shared by every bundled `CachePersistenceLike`
implementation (Memory, Deno KV, Redis, Noop): how expiration metadata is
written, how the pure-HTTP `_expiresIn()` helper is computed (RFC 9111 §4.2.1
aligned), how the public `maxPersistenceTtlMs` option bounds storage lifetime,
and how `get()` and the async iterator filter or yield stale entries depending
on the configured `staleRetention` mode (defined in the `cache-freshness-policy`
