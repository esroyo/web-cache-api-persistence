# Cache Freshness Policy Specification

## Purpose

Defines how bundled `CachePersistenceLike` implementations decide whether to
evict or retain entries once their HTTP freshness lifetime ends. The policy is
controlled at construction time via a `staleRetention` option on each
...
### Requirement: Custom maxPersistenceTtlMs SHALL be honored independently of staleRetention

The configured `maxPersistenceTtlMs` SHALL be honored by both `'evict'` and
`'retain'` modes. Under `'evict'`, it caps the HTTP-derived delay. Under
`'retain'`, it IS the delay. A custom (non-default) `maxPersistenceTtlMs` MUST
be reflected in the eviction primitive's captured argument under either mode.

#### Scenario: Custom maxPersistenceTtlMs under evict caps the HTTP-derived delay

- **WHEN** a Memory persistence with
  `{ staleRetention: 'evict', maxPersistenceTtlMs: 60_000 }` accepts a `put` for
  `Cache-Control: max-age=3600` (1 hour, exceeds 60 s), with `setTimeout` spied
- **THEN** exactly one captured `setTimeout` call has `delay === 60_000 ± 50` —
  the custom ceiling wins over the larger HTTP value

#### Scenario: Custom maxPersistenceTtlMs under retain is the delay

- **WHEN** a Memory persistence with
  `{ staleRetention: 'retain', maxPersistenceTtlMs: 60_000 }` accepts a `put`
  for `Cache-Control: max-age=3600` (1 hour, exceeds 60 s), with `setTimeout`
  spied
- **THEN** exactly one captured `setTimeout` call has `delay === 60_000 ± 50` —
  same value as the `'evict'` case above, because under `'retain'` HTTP
  expiration is not consulted for the delay

#### Scenario: Custom maxPersistenceTtlMs under evict, max-age smaller than ceiling

- **WHEN** a Memory persistence with
  `{ staleRetention: 'evict', maxPersistenceTtlMs: 60_000 }` accepts a `put` for
  `Cache-Control: max-age=30` (30 s, smaller than 60 s ceiling), with
  `setTimeout` spied
- **THEN** exactly one captured `setTimeout` call has `delay === 30_000 ± 50` —
  `min(30_000, 60_000)` = `30_000`

### Requirement: The CachePersistenceNoop implementation SHALL accept but ignore staleRetention and maxPersistenceTtlMs

`CachePersistenceNoop` MUST accept both the `staleRetention` and
`maxPersistenceTtlMs` options without error to keep the configuration surface
uniform, but MUST NOT change its behavior based on their values because it
stores nothing.

#### Scenario: noop persistence is invariant under staleRetention and maxPersistenceTtlMs

- **WHEN** two noop instances are constructed, one with
  `{ staleRetention: 'retain', maxPersistenceTtlMs: 60_000 }` and one with
  `{ staleRetention: 'evict', maxPersistenceTtlMs: 2_592_000_000 }`, and against
  each: `put(cacheName, request, response)` is awaited, then
